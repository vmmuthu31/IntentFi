...
const deposit = async ({ chainId, token, amount }) => {
  try {
    ...
    console.log(
      `Allowance for ${token}:`,
      tokenAllowance ? tokenAllowance.allowance : "0"
    );

    if (!tokenAllowance || BigInt(tokenAllowance.allowance) < BigInt(amount)) {
      console.log(`Insufficient allowance for ${token}, approving...`);
      ...
      console.log(
        `Updated allowance for ${token}:`,
        updatedTokenAllowance ? updatedTokenAllowance.allowance : "0"
      );
    ...
    const gasLimit = BigInt(200000); // Increased gas limit for deposit
    const balance = await publicClient.getBalance({ address: account.address });
...
const withdraw = async ({ chainId, token, amount }) => {
  try {
    ...
    const gasLimit = BigInt(200000); // Increased gas limit for withdraw
    const balance = await publicClient.getBalance({ address: account.address });
