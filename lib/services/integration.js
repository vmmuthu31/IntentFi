...
const signAndSendTransaction = async (txParams) => {
  try {
    ...
    if (balance.lt(totalCost)) {
      console.error(`Insufficient balance: ${ethers.utils.formatEther(balance)} < ${ethers.utils.formatEther(totalCost)}`);
      return {
        success: false,
...
const approveWithWagmi = async (req) => {
  try {
    ...
    const data = encodeFunctionData({
      ...
      args: [
        spenderAddress, 
        BigInt(ethers.utils.parseUnits(allowanceAmount.toString(), 18).toString())
      ]
    });
    ...
    console.log("Sending transaction with params:", JSON.stringify(txParams, (_, v) => 
    ...
    if (error.message && error.message.includes('insufficient funds')) {
      return {
        ...
        error: 'Insufficient funds for transaction',
        details: {
          message: 'Please fund your account with testnet CELO using the faucet',
          faucetUrl: 'https://faucet.celo.org/alfajores',
